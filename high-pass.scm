...
(define (high-pass img inLayer inRadius inMode inKeepOrig)
  (let*
    (
	  ...
	  (colours-layer 0)
	  (colours-layer2 0)
	  (orig-name (car (gimp-item-get-name inLayer)))
    )
    ;  it begins here
    (gimp-context-push)
	...
	(if (= inKeepOrig TRUE)
      (begin
	    ...
	    (gimp-image-insert-layer img working-layer -1 0)
		(gimp-item-set-name working-layer "working")
	  )
    ...
    (gimp-image-insert-layer img blur-layer -1 0)
	(gimp-item-set-name blur-layer "blur")
	...
	(if (<= inMode 3)
	  ...
	  (begin  ;else 4=redrobes method
        ...
        (gimp-image-insert-layer img colours-layer2 -1 0)		
        ...
        (gimp-image-insert-layer img blur-layer2 -1 0)		
	...
	(if (= inKeepOrig TRUE)
      (gimp-item-set-name working-layer (string-append orig-name " high pass"))
      (gimp-item-set-name working-layer orig-name)
	)   
