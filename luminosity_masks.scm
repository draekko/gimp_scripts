...
(define (script-fu-luminosity-masks image layer 
	...
	(if (not (defined? 'gimp-drawable-desaturate)) ;Check for 2.9 proc
    (begin
        (define (gimp-drawable-desaturate copied-layer DESATURATE-LUMINOSITY) ;Define it
            (gimp-desaturate-full copied-layer DESATURATE-LUMINOSITY) ;Call 2.8 proc
        )
    )
	)
	...
	(gimp-drawable-desaturate copied-layer DESATURATE-LUMINOSITY) ;{ DESATURATE-LIGHTNESS (0), DESATURATE-LUMINOSITY (1), DESATURATE-AVERAGE (2) }
...
(script-fu-register
  ...
  "<Image>/Filters/Generic/Luminosity Mask Layers"    ;menu register
  "Generates 9 layers with different luminosity masks."       ;description
  "Tin Tran / Draekko"                          ;author name
